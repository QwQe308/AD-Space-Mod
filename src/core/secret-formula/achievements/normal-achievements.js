...
export const normalAchievements = [
  ...
  },
  {
    ...
    checkRequirement: () => Time.totalTimePlayed.totalHours >= 8,
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    reward: "Small multiplier to Antimatter Dimensions based on time played.",
    effect: () => Math.max(Math.pow(Time.totalTimePlayed.totalHours, 0.1), 1),
    formatEffect: (value) => `${formatX(value, 2, 2)}`,
  },
  {
    id: 77,
  ...
  },
  {
    ...
    checkRequirement: () =>
      Currency.antimatter.exponent >= 260000 && Time.thisEternity.totalMinutes <= 1 && player.dilation.active,
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    get reward() {
  ...
  },
  {
    ...
    checkRequirement: () =>
      EternityChallenges.all.map((ec) => ec.completions).min() >= 5 && Time.thisReality.totalSeconds <= 1,
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  },
  {
    id: 164,
